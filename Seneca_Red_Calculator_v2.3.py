...
def patch_notes():
	...
	print('2.3 : very, very, very lengthy proccess of including rewriting my c++ trig calculator as a single function')
	print('  ( also added some dependent functions, cosing_law and sin_law)')
	import os 
	import glob
	...
	if (trig_input_choice == 1): 
		...
		if (trig_shape_input_choice == 1):
			...
			if (trig_sas_angleSelect.upper()=='A'): 
				...
				# I ripped the following straight out of my old calculator
				s2sq = side_2*side_2
				s3sq = round((side_3*side_3),6)
				x = cos(radians(ParentAngle))
				y = (side_2*side_3)
				temp = (2*x*y)
				PSSQ = s2sq+s3sq-temp
				ParentSide = round(sqrt(PSSQ),2)
				########## STOPPING PLACE, IT RUNS AND FINDS THE SIDE CORRECTLY
				...
				#print('B = '+str(B)+'  a = '+str(b))
				#print('A = '+str(C)+'  a = '+str(c))
